            ...
            <div class="mb-5">
                ...
                <div class="row g-5">
                    <div class="col-3">
                        ...
                        <input id="myInput" type="text" class="form-control" placeholder="Search by Job Title..">

                    </div>
                    <div class="col-3">
                        <input id="mycity" type="text" class="form-control" placeholder="Search by Location.">  
                    </div>
...
// dd($users);
                    ...
                    $collection = App\Models\enewjobs::all();
                    $num=1;
                @endphp
...
$profile = App\Models\Eprofile::where('user_id',$item->id)->first();
...
<tr class="myList mycity">
  ...
  <td>{{$item?$item->title:"Title is Empty"}}</td>
  <td>{{$item?$item->date:"Date is Empty"}}</td>
  <td>{{$item?$item->salary:"salary is Empty"}}</td>
   <td>{{$item?$item->start_time:"Start_time is Empty"}}</td>
    <td>{{$item?$item->end_time:"End Time is Empty"}}</td>
    <td>{{$profile?$profile->name:"name is empty"}}</td>
  <td>{{ $totalblog=count(\App\Models\CandiateModel::where('job_id',$item->id)->get())}}</td>
  
   <td>{{$item?$item->location:"location is Empty"}}</td>
  
  
  
  
  <td><a href="{{url('job/delete')}}/{{$item->id}}">   <button class="btn btn-sm btn-icon btn-icon-only btn-outline-danger mb-1" type="button">

...
@php
$account=Carbon\Carbon::createFromFormat('Y-m-d H:s:i',$item->created_at);
               $account_date=$account->format('y-m-d');
$date1 = Carbon\Carbon::parse($account_date);
...
<tr class="myList mycity">
  ...
  <td>{{$item?$item->email:"email is empty"}}</td>
  @if($item->category==1)
   <td>Student</td>
